...
function generateGephi(B::SparseMatrixCSC, dataName::String, suffixNames::Array{String}, queryID::Int64, suffix::String)
    ...
    CSV.write(string(folderString(FOLDER_CS_LP), join(["cslp", "node", dataName, queryID, suffix], "-"), ".csv"), df, header=true)
    exportGephiEdgelist(B[unionRes, unionRes], string(join(["cslp", "edge", dataName, queryID, suffix], "-"), ".csv"), folderString(FOLDER_CS_LP))
end
